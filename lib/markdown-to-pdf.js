...
module.exports = {
  ...
  activate: function() {
    loadDeps();
    atom.commands.add('atom-workspace', 'markdown-to-pdf:convert', this.convert);
  },
    ...
    try {
      ...
      try {
        ...
        if (err.message === 'MPP-ERROR') {
          atom.notifications.addError('Markdown-preview-plus is not supported.', {
            detail: 'Please enable markdown-preview to use fallback mode.',
          });
        } else {
          atom.notifications.addError('markdown-to-pdf: Error. Check console for more information.', {
            buttons: [
              {
