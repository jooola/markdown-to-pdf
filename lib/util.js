...
function getOutputPath(inputPath) {
  ...
  if (!inputPath) {
    ...
    // inputPath is a string
    if (!isMd(inputPath)) {
      // inputPath a non-markdown type
      atom.notifications.addWarning('Warning: File not saved as markdown type.', {
        detail: 'Attempting conversion of .{} file. \nValid extensions are `.markdown, .md, .mkd, .mkdown` and  `.ron`'.replace(
          '{}',
          inputPath.split('.').pop(),
        ),
        dismissable: true,
